...
| **マルチタブ** | Windows 11のみ（基本） | タブ切り離し・再ドッキング、複製、セッション復元を完全サポート |
...
- **タブ切り離し・再ドッキング**: タブをドラッグして新しいウィンドウへ切り離し、戻すとChrome風のゴーストタブと半透明ウィンドウで合流位置をプレビュー — 状態を完全に保持
- **タブ複製**: 正確なパスと設定でタブを複製
...
- **矢印キー & Spaceナビゲーション**: Quick Lookウィンドウ内で矢印キーとSpaceでファイルを切り替え
- **Quick Lookウィンドウサイズの記憶**: 前回のウィンドウサイズを次回起動時に自動復元
...
| Delete | ごみ箱に移動 |
| Ctrl+Tab / Ctrl+Shift+Tab | タブの切り替え（次/前） |
| F6 | 分割ビューのパネル切り替え |
...
### タブのドラッグ&ドロップUX *(v1.2.13.0)*

![タブの切り離し・再ドッキング](tab-drag.gif)

- Chrome風のゴーストタブインジケーターで合流先を視覚的に表示
- 半透明のドッキングフィードバックで挿入位置を確認
- タブ切り離し時のビジュアルエフェクト（タブが1つの場合は無効化）
- 安定した固定幅タブでドラッグ中のレイアウト崩れを防止
