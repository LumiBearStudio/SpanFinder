...
| **멀티 탭** | Windows 11만 (기본) | 탭 분리, 재도킹, 복제, 세션 복원 완전 지원 |
...
- **탭 분리 & 재도킹**: 탭을 드래그하여 새 창으로 분리, 다시 드래그하여 재도킹 — Chrome 스타일 고스트 탭 인디케이터와 반투명 창 피드백
...
- **방향키 & Space 탐색**: Quick Look을 닫지 않고 파일 간 이동
- **창 크기 기억**: Quick Look이 마지막 크기를 세션 간 유지
...
| Ctrl+Tab / Ctrl+Shift+Tab | 탭 순환 (앞/뒤) |
...
### 탭 드래그 & 드롭 UX *(v1.2.13.0)*

![탭 분리 & 재도킹](tab-drag.gif)

- **Chrome 스타일 고스트 탭**: 드래그한 탭이 어디에 위치할지 시각적으로 표시
- **반투명 도킹 피드백**: 재도킹 시 대상 창이 반투명으로 변경
- **분리 시각 효과**: 탭을 밖으로 드래그할 때 투명도 및 들어올림 애니메이션 (단일 탭 창에서는 비활성화)
- **안정적인 탭 너비**: 폴더 이름 길이에 따라 변하지 않는 Chrome 스타일 고정 너비 탭
