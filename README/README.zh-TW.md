...
| **多分頁** | 僅 Windows 11（基本） | 分頁拖出與重新停靠、複製、工作階段完整還原 |
| **分割檢視** | 無 | 獨立檢視模式的雙窗格 |
...
- **分頁拖出與重新停靠**：將分頁拖曳至新視窗進行拆分，拖回時透過 Chrome 風格的幽靈分頁與半透明視窗預覽停靠位置——狀態完整保留
...
- **方向鍵與空白鍵瀏覽**：在 Quick Look 視窗中使用方向鍵與空白鍵切換檔案
- **Quick Look 視窗大小記憶**：自動還原上次使用的視窗尺寸
...
| Ctrl+Shift+E | 切換分割檢視 |
| Delete | 移至資源回收桶 |
| Ctrl+Tab / Ctrl+Shift+Tab | 切換分頁（下一個/上一個） |
| F6 | 切換分割檢視窗格 |
...
![分頁分離與重新停靠](tab-drag.gif)

- Chrome 風格的幽靈分頁指示器，直覺顯示停靠位置
- 半透明停靠回饋，確認插入位置
- 分頁拆分視覺效果（僅一個分頁時自動停用）
- 穩定的固定寬度分頁，拖曳過程中版面不會跳動
