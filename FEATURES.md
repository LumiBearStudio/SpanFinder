...
| 동작 | 설명 |
|------|------|
| 탭 드래그 (창 바깥) | 새 창으로 탭 분리 (Tear-Off) — 투명도+들어올림 시각 효과 |
| 탭 드래그 (다른 창) | 대상 창에 탭 재도킹 — 고스트 탭 인디케이터 + 반투명 창 피드백 |
| 탭 드래그 (같은 창 내) | 탭 순서 변경 |
| 단일 탭 드래그 | 창 이동 (분리 효과 없음) |

### 워크스페이스 (v1.2.1.0)

| 단축키 | 동작 |
|--------|------|
| `Ctrl+Shift+S` | 현재 탭 레이아웃 저장 |
...
- Quick Look 방향키(←→) / Space 파일 탐색
- Quick Look 창 크기 세션 간 유지
- Markdown 렌더링 (다크/라이트 테마)
- CSV/TSV 테이블 뷰 (최대 200행)
